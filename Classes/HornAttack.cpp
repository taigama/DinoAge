...
bool HornAttack::init()
{
	if (!Projectile::init())
		return false;
	// sprite init & physic init were called in base

	_projectileType = PROJECTILE_TYPE::HORN_ATTACK;
...
void HornAttack::update(float delta)
{
	if (_timer <= 0)
	{
...
void HornAttack::reset(float params[10])
{
	this->setActive(true);
	_timer = ATTACK_TIME;
...
void HornAttack::attack(Character* target, Character::DIRECTION dir)
{
	// make enemy hurted
