...
bool HiddenBlock::init()
{
	if (!Block::init())
		return false;

	_blockType = BLOCK_TYPE::HIDDEN;
...
void HiddenBlock::update(float delta)
{
	if (m_isFlash)
	{
		m_timerFLash -= delta;
...
void HiddenBlock::init(bool isHidden, bool isOneWay
	, int x1, int y1
	, int x2, int y2
	, int zone_ID)
{
...
bool HiddenBlock::onContactBegin(const cocos2d::PhysicsContact &contact)
{
...
bool HiddenBlock::onContactSeparate (const cocos2d::PhysicsContact &contact)
{
	// Retrieves PhysicsBody of target
	auto targetBody = contact.getShapeA()->getBody();
