...
bool EnemyWerewolf::physicsRayCastFunc(cocos2d::PhysicsWorld& world, const cocos2d::PhysicsRayCastInfo& info, void* data)
{
	...
	PhysicsBody* targetBody = info.shape->getBody();
	GameObject* target = (GameObject*) targetBody->getOwner();

	if (target->getType() != GameObject::OBJECT_TYPE::BLOCK && target->getType() != GameObject::OBJECT_TYPE::CHARACTER)
		return true;


	Character* charTarget = (Character*)target;
	if (charTarget->getTeam() == _team)
		return true;

	// Stores all PhysicsBodies detected
	_bodies.pushBack(targetBody);
